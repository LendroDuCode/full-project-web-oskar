...
const PublicationBadge = ({ estPublie }: { estPublie: boolean }) => {
  return estPublie ? (
    <span
    ...
    >
      <FontAwesomeIcon icon={faCheckCircle} className="fs-11" />
      Publié
    </span>
  ) : (
    <span
    ...
    >
      <FontAwesomeIcon icon={faTimesCircle} className="fs-11" />
      Non publié
    </span>
  );
};
...
export default function DonVendeurPage() {
  // États
  const [dons, setDons] = useState<DonVendeur[]>([]);
  ...
  const [selectedDon, setSelectedDon] = useState<DonVendeur | null>(null);
    ...
    async (params?: Record<string, any>) => {
      try {
        ...
        const endpoint = queryParams.toString()
          ? `${API_ENDPOINTS.DONS.VENDEUR_DONS}?${queryParams.toString()}`
          : API_ENDPOINTS.DONS.VENDEUR_DONS;

        const response = await api.get<{ status: string; data: DonVendeur[] }>(
          endpoint,
        );
        ...
        console.error("❌ Erreur lors du chargement des dons vendeur:", err);
  ...
  }, [searchTerm, selectedStatus, selectedPublication, fetchDons]);

  // Fonctions de tri
  const requestSort = (key: keyof DonVendeur) => {
  ...
  const getSortIcon = (key: keyof DonVendeur) => {
  ...
  }, [sortedDons, searchTerm, selectedStatus, selectedPublication]);

  // Statistiques
  const stats = useMemo(() => {
    const totalDons = dons.length;
    const publishedDons = dons.filter((d) => d.estPublie).length;
    const availableDons = dons.filter((d) => d.statut === "disponible").length;
    const blockedDons = dons.filter((d) => d.est_bloque).length;

    return {
      total: totalDons,
      published: publishedDons,
      available: availableDons,
      blocked: blockedDons,
    };
  }, [dons]);
  ...
  const openDeleteModal = (don: DonVendeur) => {
    setSelectedDon(don);
  ...
  const handlePublishDon = async (donUuid: string) => {
    try {
      await api.post(API_ENDPOINTS.DONS.PUBLISH, { uuid: donUuid });
  ...
  const handleUnpublishDon = async (donUuid: string) => {
    try {
      // Note: Vous devrez créer cet endpoint dans votre API
      await api.post(`${API_ENDPOINTS.DONS.DETAIL(donUuid)}/unpublish`);
      handleSuccess("Don dépublifié avec succès !");
    } catch (err: any) {
      setError(
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.download = `mes-dons-${new Date().toISOString().split("T")[0]}.pdf`;
  ...
  const createCSVExport = () => {
    ...
    const csvContent = [
      ...
      ...dons.map((don) => [
        ...
        `"${don.categorie || ""}"`,
        `"${don.localisation || ""}"`,
        don.statut || "disponible",
        don.estPublie ? "Publié" : "Non publié",
        don.est_bloque ? "Bloqué" : "Non bloqué",
        `"${don.vendeur || ""}"`,
        `"${don.lieu_retrait || ""}"`,
        don.createdAt
    ...
    link.setAttribute(
      "download",
      `mes-dons-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handleBulkAction = async (
    ...
    try {
      ...
      setSuccessMessage(
        `${successCount} don(s) ${action === "publish" ? "publié(s)" : "dépublié(s)"} avec succès${errorCount > 0 ? ` (${errorCount} échec(s))` : ""}`,
      );
      setTimeout(() => setSuccessMessage(null), 3000);
    ...
    <>
      ...
      <div className="p-3 p-md-4">
        <div className="card border-0 shadow-sm overflow-hidden">
          ...
          <div className="card-header text-white border-0 py-3">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
              <div>
                ...
                <div className="d-flex align-items-center gap-3">
                  <h2 className="h4 mb-0 fw-bold text-dark">Mes Dons</h2>
          ...
          {selectedDons.length > 0 && (
            <div className="p-3 border-bottom bg-info bg-opacity-10">
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                <div className="d-flex align-items-center gap-2">
                  <FontAwesomeIcon icon={faGift} className="text-info" />
              ...
              <div className="text-center py-5">
                ...
                >
                  ...
                  <p className="mb-0 text-center">
                    {dons.length === 0
                      ? "Vous n'avez pas encore créé de don. Commencez par en créer un !"
                ...
                <table className="table table-hover mb-0">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th style={{ width: "150px" }}>
                        <span className="fw-semibold">Statuts</span>
                      </th>
                      <th style={{ width: "180px" }}>
                        <button
                          className="btn btn-link p-0 text-decoration-none fw-semibold text-dark border-0 bg-transparent"
                      ...
                      >
                              ...
                              <img
                                ...
                                onError={(e) => {
                                  (e.target as HTMLImageElement).src =
                                    `https://via.placeholder.com/60/0d6efd/ffffff?text=${don.nom.charAt(0)}`;
                                }}
                              />
                            </div>
                          ) : (
                            <div
                              className="bg-info bg-opacity-10 rounded d-flex align-items-center justify-content-center mx-auto"
                              style={{ width: "60px", height: "60px" }}
                            >
                              <FontAwesomeIcon
                                icon={faGift}
                        ...
                        <td>
                          <div className="d-flex flex-column gap-2">
                            ...
                            />
                            <PublicationBadge estPublie={don.estPublie} />
                          </div>
                        </td>
                        <td>
                          <div className="d-flex flex-column">
                            <div className="d-flex align-items-center">
                              ...
                              />
                              <small className="text-muted">
                                {formatDate(don.updatedAt || don.createdAt)}
                              </small>
                            </div>
                            {don.date_debut && (
                              <div className="mt-1">
