...
type BulkActionType = "unblock" | "delete" | "export";
            ...
            {echange && (
              <>
                <p className="mb-3">
                  ...
                  <strong>{echange.nomElementEchange}</strong> ?
                </p>
        ...
        <div className="modal-content border-0 shadow-lg">
          ...
          <div className="modal-body p-4">
            <div className="alert alert-warning mb-3 border-0">
              ...
              {selectedCount} échange(s) bloqué(s) sélectionné(s).
            </div>
...
const ConfirmBulkActionModal = ({
  ...
  const getActionMessage = () => {
    switch (action.type) {
      case "unblock":
        return `débloquer ${selectedCount} échange(s)`;
      case "delete":
          ...
          <div className="modal-body p-4">
            ...
            {action.type === "unblock" && (
              <div className="text-success">
                <small>
                  <strong>Note :</strong> Les échanges débloqués redeviendront
                  visibles et pourront être traités.
                </small>
              </div>
            )}
          </div>
...
export default function ListeEchangeBloques() {
  // États
  ...
  const [selectedEchange, setSelectedEchange] = useState<EchangeBloque | null>(
    null,
  );
  ...
  const bulkActions: BulkAction[] = [
    {
      ...
      icon: faLockOpen,
      color: colors.oskar.green,
      description: "Débloquer les échanges sélectionnés",
    },
    {
      type: "export",
      label: "Exporter",
  ...
  // Charger les échanges bloqués de l'utilisateur
  const fetchEchangesBloques = useCallback(async () => {
    try {
      ...
      console.log("📡 Chargement des échanges bloqués de l'utilisateur...");
      ...
      console.error("❌ Erreur lors du chargement des échanges bloqués:", err);
  ...
  const filteredEchanges = useMemo(() => {
    return echanges.filter((echange) => {
      const matchesSearch =
        ...
        echange.message?.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      const matchesType =
        selectedType === "all" ||
        echange.typeEchange?.toLowerCase() === selectedType.toLowerCase();

      return matchesSearch && matchesStatus && matchesType;
    });
  }, [echanges, searchTerm, selectedStatus, selectedType]);
  ...
  const openDeleteModal = (echange: EchangeBloque) => {
    setSelectedEchange(echange);
    setShowDeleteModal(true);
  };

  // Débloquer un échange
  const handleUnblock = async (echange: EchangeBloque) => {
    try {
      // Note: L'API ne semble pas avoir d'endpoint spécifique pour débloquer
      // On peut utiliser l'endpoint UPDATE avec un statut différent
      await api.put(API_ENDPOINTS.ECHANGES.UPDATE(echange.uuid), {
        ...echange,
        est_bloque: false,
        statut: "en_attente", // Remettre en attente
      });

      handleSuccess("Échange débloqué avec succès !");
    } catch (err: any) {
  ...
  const exportSelectedEchanges = (echangesList: EchangeBloque[]) => {
    try {
      ...
      const csvData = echangesList.map((echange) => [
        ...
        echange.objetPropose,
        echange.objetDemande,
        echange.typeEchange === "produit" ? "Produit" : "Service",
        echange.statut,
        formatPrice(echange.prix),
        echange.categorie,
        formatDate(echange.dateProposition),
        echange.message?.replace(/,/g, ";") || "",
        echange.agent,
        echange.utilisateur,
        echange.vendeur,
      ]);
      ...
      link.setAttribute(
        "download",
        `echanges_bloques_export_${new Date().toISOString().split("T")[0]}_${echangesList.length}_elements.csv`,
      );
  ...
  const formatPrice = (price?: string) => {
    if (!price || price === "0.00") return "Gratuit";
  ...
  const stats = useMemo(
    () => ({
      ...
      produit: echanges.filter(
        (e) => e.typeEchange?.toLowerCase() === "produit",
      ).length,
      service: echanges.filter(
        (e) => e.typeEchange?.toLowerCase() === "service",
      ).length,
    }),
    [echanges],
  );
        ...
        <div className="card border-0 shadow-sm overflow-hidden">
          <div className="card-header bg-white border-0 py-3">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
              <div>
                <p className="text-muted mb-1">Échanges Bloqués</p>
                <div className="d-flex align-items-center gap-3">
                  <h2 className="h4 mb-0 fw-bold">Mes Échanges Bloqués</h2>
                  <span className="badge bg-danger bg-opacity-10 text-danger">
                    <FontAwesomeIcon icon={faBan} className="me-1" />
                    {stats.total} échange(s) bloqué(s)
                  </span>
              ...
              <div className="text-center py-5">
                ...
                >
                  ...
                  <h5 className="alert-heading">
                    {echanges.length === 0
                      ? "Aucun échange bloqué"
                      : "Aucun résultat"}
                  </h5>
                  <p className="mb-0 text-center">
                    {echanges.length === 0
                      ? "Tous vos échanges sont actifs et disponibles."
                      ...
                      >
                              ...
                              <img
                                ...
                                onError={(e) => {
                                  (e.target as HTMLImageElement).src =
                                    `https://via.placeholder.com/60/6C757D/FFFFFF?text=${echange.nomElementEchange?.charAt(0) || "E"}`;
                                }}
                              />
                              <div className="position-absolute top-0 start-0 end-0 bottom-0 bg-danger bg-opacity-25 rounded"></div>
                            </div>
                          ) : (
                            <div
                        ...
                        <td>
                          <div className="d-flex flex-column gap-1">
                            ...
                            <div>
                              ...
                              <div className="fw-semibold">
                                {echange.objetDemande || "Non spécifié"}
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td>
                          <div className="d-flex flex-column gap-1">
                            ...
                            <div>
                              <small className="text-muted">Assigné à:</small>
                              <div className="fw-semibold">
                                <FontAwesomeIcon
                                  icon={faUserTie}
                                  className="me-1"
                                />
                                {echange.agent || "Non assigné"}
                              </div>
                            </div>
                          </div>
                        </td>
        ...
        {selectedEchanges.size > 0 && (
          <div className="position-fixed bottom-0 start-50 translate-middle-x mb-4">
            <div className="bg-white border rounded-4 shadow-lg p-3">
              <div className="d-flex align-items-center gap-3">
                <div className="d-flex align-items-center gap-2">
                  <div className="bg-warning bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center p-2">
                    <FontAwesomeIcon
                      icon={faCheckSquare}
                  ...
                  <div>
                    <div className="fw-bold text-warning">
                      {selectedEchanges.size} échange(s) bloqué(s)
                      sélectionné(s)
                    </div>
                    <small className="text-muted">
