...
interface ApiResponse {
  status: string;
  data: EchangeUtilisateur[];
}

// Types pour les actions groupées
type BulkActionType = "publish" | "delete" | "duplicate" | "export";
...
const DeleteModal = ({
  ...
  show: boolean;
  echange: EchangeUtilisateur | null;
  loading: boolean;
  onClose: () => void;
  onConfirm: () => void;
}) => {
...
export default function ListeEchangeUtilisateur() {
  // États
  ...
  const [selectedEchange, setSelectedEchange] =
    useState<EchangeUtilisateur | null>(null);
  ...
  const fetchEchangesUtilisateur = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      setInfoMessage(null);
      ...
      const response = await api.get<ApiResponse>(
        API_ENDPOINTS.ECHANGES.USER_ECHANGES,
      );
      ...
      let errorMessage = "Erreur lors du chargement de vos échanges.";
  ...
  // Fonctions de tri
  const requestSort = (key: keyof EchangeUtilisateur) => {
  ...
  const getSortIcon = (key: keyof EchangeUtilisateur) => {
  ...
  const openDeleteModal = (echange: EchangeUtilisateur) => {
    setSelectedEchange(echange);
    setShowDeleteModal(true);
  };

  // Publier un échange
  const handlePublish = async (echange: EchangeUtilisateur) => {
    try {
        ...
        case "duplicate":
          for (const echange of selectedEchangesList) {
            const duplicatedEchange = {
              nomElementEchange: `${echange.nomElementEchange} (Copie)`,
  ...
  const exportSelectedEchanges = (echangesList: EchangeUtilisateur[]) => {
    try {
      ...
      link.setAttribute(
        "download",
        `mes_echanges_export_${new Date().toISOString().split("T")[0]}_${echangesList.length}_elements.csv`,
      );
        ...
        <div className="card border-0 shadow-sm overflow-hidden">
          <div className="card-header bg-white border-0 py-3">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
              <div>
                <p className="text-muted mb-1">Mes Échanges</p>
                <div className="d-flex align-items-center gap-3">
                  <h2 className="h4 mb-0 fw-bold">Mes Échanges Personnels</h2>
              ...
              <div className="d-flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => fetchEchangesUtilisateur()}
                ...
                <table className="table table-hover mb-0">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th style={{ width: "150px" }}>
                        <span className="fw-semibold">Statut & Type</span>
                      </th>
                      <th style={{ width: "150px" }}>
                        <button
                          className="btn btn-link p-0 text-decoration-none fw-semibold text-dark border-0 bg-transparent"
                              ...
                              <img
                                ...
                                onError={(e) => {
                                  (e.target as HTMLImageElement).src =
                                    `https://via.placeholder.com/60/4A90E2/FFFFFF?text=${echange.nomElementEchange?.charAt(0) || "E"}`;
                                }}
                              />
                            </div>
                          ) : (
                            ...
                            <div className="mt-1">
                              ...
                              >
                                <FontAwesomeIcon
                                  ...
                                  className="me-1"
                                />
                                {echange.message.length > 20
                                  ? `${echange.message.substring(0, 20)}...`
                                  : echange.message}
                              </small>
                            </div>
                          )}
