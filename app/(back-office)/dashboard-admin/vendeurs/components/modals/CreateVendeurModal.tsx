...
interface CreateVendeurModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export default function CreateVendeurModal({
  isOpen,
  onClose,
  onSuccess,
  ...
  // Styles personnalisés avec la couleur verte Oskar
  const styles = {
    modalHeader: {
      background: `linear-gradient(135deg, ${colors.oskar.green} 0%, ${colors.oskar.green}80 100%)`,
      borderBottom: `3px solid ${colors.oskar.greenHover || "#15803d"}`,
    },
    sectionHeader: {
    ...
    primaryButtonHover: {
      background: colors.oskar.greenHover || "#15803d",
      borderColor: colors.oskar.greenHover || "#15803d",
    },
    secondaryButton: {
      background: "white",
      color: colors.oskar.green,
    ...
    secondaryButtonHover: {
      background: colors.oskar.lightGrey,
      color: colors.oskar.greenHover || "#15803d",
      borderColor: colors.oskar.greenHover || "#15803d",
    },
  };
  ...
  const loadOptions = useCallback(async () => {
    try {
      ...
      console.log("🔄 Loading civilites and roles for vendeur...");
      ...
      // Filtrer les rôles pour vendeurs (role Vendeur ou contenant "vendeur")
      const vendeurRoles = rolesData.filter(
        (role) =>
          role.name.toLowerCase().includes("vendeur") ||
          role.name === "Vendeur" ||
          role.feature === "vendeur",
      );

      console.log(
        "🎯 Filtered vendeur roles:",
        vendeurRoles.length,
        vendeurRoles,
      );
      setRoles(vendeurRoles);
      ...
      if (vendeurRoles.length > 0) {
        const defaultVendeurRole =
          vendeurRoles.find((r) => r.name === "Vendeur") || vendeurRoles[0];
        setFormData((prev) => ({
          ...prev,
          role_uuid: defaultVendeurRole.uuid,
        }));
      }

      console.log(
        `✅ Chargement réussi: ${civilitesData.length} civilités, ${vendeurRoles.length} rôles vendeur`,
      );
    } catch (err: any) {
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      const vendeurRole = roles.find((r) => r.name === "Vendeur");
      const defaultRoleUuid = vendeurRole
        ? vendeurRole.uuid
        : roles.length > 0
          ? roles[0].uuid
          : "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Préparer les données pour l'API (exactement comme dans l'exemple JSON)
      const vendeurData = {
        ...
        indicatif: "+225",
        code_utilisateur: `VEND_${Date.now()}`,
        statut: "actif",
        adminUuid: "0", // À remplacer par l'UUID de l'admin connecté
      };

      console.log("📤 Envoi des données pour création vendeur:", vendeurData);
      ...
      console.log("✅ Vendeur créé:", response.data);
      ...
      let errorMessage = "Erreur lors de la création du vendeur";
  ...
  const generatePassword = () => {
    ...
    // Remplir le reste (total 12 caractères)
    const allChars =
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="createVendeurModalLabel"
      aria-modal="true"
    >
          ...
          >
            <div className="d-flex align-items-center">
              <div className="bg-white bg-opacity-20 rounded-circle p-2 me-3">
                <FontAwesomeIcon icon={faStore} className="fs-5" />
              </div>
              <div>
                <h5
                  className="modal-title mb-0 fw-bold"
                  id="createVendeurModalLabel"
                >
                  ...
                  <p className="mb-3">
                    {civilites.length === 0 && roles.length === 0
                      ...
                      : civilites.length === 0
                        ...
                        : "Impossible de charger les rôles vendeur."}
                  </p>
                    ...
                    <div className="d-flex align-items-center">
                      ...
                      <div>
                        <h6
                          ...
                          style={{ color: colors.oskar.green }}
                        >
                          Type et Rôle du Vendeur
                        </h6>
                        <small className="text-muted">
                          Définir le type et les permissions du vendeur
                        </small>
                      </div>
                    </div>
                  </div>
                  <div className="card-body p-4">
                    <div className="row g-4">
                      ...
                      <div className="col-md-6">
                        ...
                        <small className="text-muted">
                          Détermine les permissions et fonctionnalités
                        </small>
                      </div>
                      ...
                      <div className="col-md-6">
                        ...
                        <small className="text-muted">
                          Rôle attribué au vendeur dans le système
                        </small>
                      </div>
                    </div>
                  </div>
