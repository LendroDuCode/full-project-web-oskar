// CTABanner.tsx - CORRIGÉ avec conversion de type correcte
"use client";
...
// Interface pour le type d'utilisateur du contexte Auth
interface AuthUser {
  uuid?: string;
  email?: string;
  nom_complet?: string;
...
const CTABanner = () => {
  ...
  const { isLoggedIn, openLoginModal, user: authUser } = useAuth();

  // Convertir l'utilisateur du contexte au format UserInfo attendu
  const userInfo: UserInfo | undefined = authUser ? {
    uuid: authUser.uuid || '',
    email: authUser.email || '',
    nom_complet: authUser.nom_complet || authUser.nom || '',
    type: authUser.type || 'utilisateur',
    role: authUser.role,
    temp_token: authUser.temp_token,
    userData: {
      type: authUser.type || 'utilisateur'
    }
  } : undefined;
