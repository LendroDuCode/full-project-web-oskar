...
const transformDonSimilaireData = (apiSimilaire: DonSimilaireAPI): DonSimilaire => {
  let imageUrl = "";
...
export default function DonDetailPage() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: "Le donateur vous indiquera le lieu et l'heure de retrait. Rencontrez-vous dans un lieu public et vérifiez l'article avant de l'accepter.",
    },
    {
      question: "Est-ce vraiment gratuit ?",
      answer: "Oui, les dons sont entièrement gratuits. Aucun paiement n'est requis. Si quelqu'un vous demande de l'argent, signalez-le immédiatement.",
    },
    {
      question: "Que faire si le donateur ne répond pas ?",
      answer: "Vous pouvez essayer de contacter le donateur via plusieurs moyens (WhatsApp, messagerie). S'il ne répond toujours pas, vous pouvez consulter d'autres dons similaires.",
    },
    {
      question: "Puis-je annuler ma demande de don ?",
      answer: "Oui, vous pouvez annuler votre demande à tout moment avant le retrait. Nous vous recommandons d'informer le donateur de votre décision.",
    },
  ];
    ...
    requireAuth(() => {
      ...
      if (isMobile) {
        ...
        const params = new URLSearchParams({
          ...
          destinataireEmail: createur.email || "",
          destinataireNom: `${createur.prenoms} ${createur.nom}`,
          sujet: `Intéressé(e) par votre don: ${don.nom}`,
          donUuid: don.uuid,
          donImage: don.image || "",
          donNom: don.nom || "",
        });
    ...
    requireAuth(async () => {
      ...
      try {
        if (favori && favoriteId) {
          ...
          const payload = { itemUuid: don.uuid, type: "don" };
    ...
    requireAuth(() => {
      if (window.confirm("Signaler ce commentaire comme inapproprié ?")) {
        api.post(API_ENDPOINTS.COMMENTAIRES.REPORT(commentUuid), {})
          .then(() => {
            showToast("success", "Commentaire signalé. Notre équipe le vérifiera sous 24h.");
          })
          .catch((err) => {
  ...
  // ============================================
  const fetchCommentaires = useCallback(async (donUuid: string) => {
    ...
    try {
      ...
      if (response && Array.isArray(response)) {
        ...
        const transformed = selected.map((item: any) => ({
          ...
          ...(item.createur && { createur: transformCreateurInfo(item.createur) }),
        }));
  ...
  const fetchDonDetails = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await api.get<DonResponse>(API_ENDPOINTS.DONS.RANDOM_DETAIL(uuid));
      if (!response || !response.don) throw new Error("Don non trouvé");
      ...
      while (imageUrls.length < 5) imageUrls.push(getDefaultDonImage());
  ...
  useEffect(() => {
    if (uuid && !initialLoadDone.current) fetchDonDetails();
  ...
  const condition = getConditionBadge(don?.statut || "disponible");
  const visibleComments = showMoreComments ? commentaires : commentaires.slice(0, 3);
  const donsAShow = donsSimilaires.length > 0 ? donsSimilaires.slice(0, 4) : donsRecents.slice(0, 4);
            ...
            <div className="col-md-8 col-lg-6 text-center">
              <div className="card border-0 shadow-lg rounded-4">
                <div className="card-body py-5">
                  <div className="mb-4">
                    <FontAwesomeIcon icon={faGift} className="fa-4x text-muted" />
                  </div>
                  <h1 className="h3 mb-3">Don introuvable</h1>
                  <p className="text-muted mb-4">{error || "Ce don n'existe pas ou a été supprimé."}</p>
                  <div className="d-flex gap-3 justify-content-center">
                    <Link href="/dons" className="btn btn-outline-primary rounded-pill px-4">
                  ...
                  <div className="btn-group" style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.15)" }}>
                    ...
                    {showShareMenu && (
                      <div className="position-absolute top-100 start-50 translate-middle-x mt-2 bg-white rounded-3 shadow-lg p-2" style={{ minWidth: "200px", zIndex: 10 }}>
                        ...
                        <button className="btn btn-link w-100 text-start text-decoration-none p-2 hover-bg-light rounded-2" onClick={() => handleShare("whatsapp")}>
                          <FontAwesomeIcon icon={faWhatsapp} className="text-success me-2" />
                          WhatsApp
                        </button>
                        <button className="btn btn-link w-100 text-start text-decoration-none p-2 hover-bg-light rounded-2" onClick={() => handleShare("tiktok")}>
                          <FontAwesomeIcon icon={faTiktok} className="text-dark me-2" />
                          TikTok
                        </button>
                        <button className="btn btn-link w-100 text-start text-decoration-none p-2 hover-bg-light rounded-2" onClick={() => handleShare("linkedin")}>
                          <FontAwesomeIcon icon={faLinkedinIn} className="text-info me-2" />
                          LinkedIn
                        </button>
                        <div className="dropdown-divider"></div>
                        <button className="btn btn-link w-100 text-start text-decoration-none p-2 hover-bg-light rounded-2" onClick={handleCopyLink}>
              ...
              {loadingComments ? (
                ...
                <>
                  ...
                  {showAddReview ? (
                    ...
                    <div className="text-center mb-4">
                      <button className="btn btn-primary rounded-pill px-4" onClick={() => { if (!isLoggedIn) openLoginModal(); else setShowAddReview(true); }}>
                  ...
                  {commentaires.length > 3 && (
                    <button className="w-100 mt-4 btn btn-outline-warning py-3 fw-semibold" onClick={() => setShowMoreComments(!showMoreComments)}>
          ...
          <div className="col-lg-4">
            ...
            <div className="card border-0 shadow-lg rounded-4 p-4 sticky-top" style={{ top: "100px" }}>
              ...
              <div className="border-top pt-4 mb-4">
                <button onClick={handleAddToFavorites} className="btn btn-light w-100 py-3 fw-semibold">
              ...
              <div className="small">
                ...
                <div className="d-flex justify-content-between">
                  ...
                  <span className={`fw-semibold ${don.disponible ? "text-success" : "text-danger"}`}>{don.disponible ? "Disponible" : "Non disponible"}</span>
                </div>
              </div>
            ...
            <div className="card border-0 shadow-lg rounded-4 p-4 mt-4 border-success">
              ...
              <ul className="list-unstyled mb-0">
                ...
                <li><FontAwesomeIcon icon={faCheckCircle} className="text-success me-2" />Aucun paiement</li>
              </ul>
